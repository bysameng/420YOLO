...
public class vp_FPInput : MonoBehaviour
{
	...
	protected virtual void Update()
	{
		...
		// ignore gameplay input if not allowed, or if game is paused
		if (!m_AllowGameplayInput || (Player.Pause.Get() == true))
			return;
	...
	protected virtual void InputMove()
	{
		...
		// NOTES:
		//  1) you may also use 'GetAxis', but that will add smoothing
		// to the input from both Ultimate FPS Camera and from Unity,
		// and might require some tweaking in order not to feel laggy
		//  2) notice how the 'MoveVector' is exposed to the player
		// event handler via the 'OnValue_InputMoveVector' property
		// below. this is how e.g. the vp_FPController class gets hold
		// of it

		m_MoveVector.x = Input.GetAxisRaw("Horizontal");
		m_MoveVector.y = Input.GetAxisRaw("Vertical");

		// keep controller from moving faster diagonally
