...
public class vp_FPCamera : vp_Component
{
	...
	// rotation counter
	public RotationCounter myRotationCounter;
	private float m_rotationDelta;

	// character controller of the parent gameobject
	public vp_FPController FPSController = null;
	...
	protected override void Awake()
	{
		...
		FPSController = Root.GetComponent<vp_FPController>();
	...
	protected override void Start()
	{
		...
		base.Start();
		myRotationCounter = GameObject.Find("RotationCounterObject").GetComponent<RotationCounter>();
		Refresh();
	...
	protected override void LateUpdate()
	{
		...
		m_Transform.position = FPSController.SmoothPosition;
		...
		myRotationCounter.UpdateRotation(m_rotationDelta);
	...
	protected virtual void DoCameraCollision()
	{
		...
		m_CameraCollisionStartPos = FPSController.Transform.TransformPoint(0, PositionOffset.y, 0);
		...
		m_CameraCollisionEndPos = Transform.position + (Transform.position - m_CameraCollisionStartPos).normalized * FPSController.CharacterController.radius;
		...
		if (Physics.Linecast(m_CameraCollisionStartPos, m_CameraCollisionEndPos, out m_CameraHit, vp_Layer.Mask.ExternalBlockers))
		{
			if (!m_CameraHit.collider.isTrigger)
				Transform.position = m_CameraHit.point - (m_CameraHit.point - m_CameraCollisionStartPos).normalized * FPSController.CharacterController.radius;
		}
	...
	protected virtual void UpdateMouseLook()
	{
		...
		MouseSmoothSteps = Mathf.Clamp(MouseSmoothSteps, 0, 20);
		...
		m_Yaw += input.x * (MouseSensitivity.x);
		m_rotationDelta = input.x * (MouseSensitivity.x);
		m_Pitch -= input.y * (MouseSensitivity.y);
	...
	protected virtual void UpdateBob()
	{
		...
		m_BobSpeed = (FPSController.Grounded ? FPSController.CharacterController.velocity.sqrMagnitude : 0.0f);
	...
	protected virtual void UpdateSwaying()
	{
		...
		Vector3 localVelocity = Transform.InverseTransformDirection(FPSController.CharacterController.velocity * 0.016f) * Time.timeScale;
	...
	/// </summary>
	protected virtual void SetRotation(Vector2 eulerAngles)
	{
