...
public class vp_FPController : vp_Component
{
	...
	public float MotorMinJumpDelay = 0.0f;
	...
	public Vector3 SmoothPosition { get { return m_SmoothPosition; } }	// a version of the controller position calculated in 'Update'
	...
	protected override void FixedUpdate()
	{
		...
		// convert user input to motion in the cardinal directions
		UpdateMotor();

		// apply motion generated by tapping or holding the jump button
		UpdateJump();

		// handle external forces such as gravity, explosion shockwaves or wind
		UpdateForces();
	...
	protected virtual void UpdateMotor()
	{
		...
		// snap super small values to zero (avoiding some floating point bugs)
	...
	/// <summary>
	/// handles tap-jumping and jumping with the button held down
	/// </summary>
		...
		if (Player.Jump.Active)
		{
			...
			if (!m_Grounded)
			{
				...
				if (m_MotorJumpForceHoldSkipFrames > 2)
				{
					...
					if (!(m_CharacterController.velocity.y < 0.0f /*&& m_JumpsAllowedOnTouchDown*/))
	...
	protected virtual void FixedMove()
	{
		...
		// data on the object directly below us, ground angle etc.
		Physics.SphereCast(new Ray(Transform.position + Vector3.up * (m_CharacterController.radius), Vector3.down),
		(m_CharacterController.radius), out m_GroundHit, (m_SkinWidth + 0.001f), vp_Layer.Mask.ExternalBlockers);
		...
		if (!m_Grounded && (m_CharacterController.velocity.y > 0.0f))
		{
			...
			m_CharacterController.radius, out m_CeilingHit, m_CharacterController.height - (m_CharacterController.radius - m_SkinWidth) + 0.01f,
		...
		if (!m_Grounded && m_WasGroundedLastFrame)
		{

			// if we lost grounding during this move, undo anti-bump offset to make the fall smoother
	...
	protected virtual void UpdateCollisions()
	{
		...
		if (m_Grounded && !m_WasGroundedLastFrame)
		{
			...
			m_FallImpact = -m_HighestFallSpeed * Time.timeScale;		// store fall impact
			...
			m_FallImpact = 0.0f;
		m_WasGroundedLastFrame = m_Grounded;
	...
	/// <summary>
	/// sets the position of the FPSController
	/// </summary>
	...
	public virtual void DeflectDownForce()
	{
		...
		if (GroundAngle > 85)
		{
			m_MotorThrottle += (Vector3.Scale((Vector3.right + Vector3.forward), (GroundNormal * m_FallImpact)));
			m_Grounded = false;
		}
